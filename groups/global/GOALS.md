...
Current theses:
1. **Unscopable agent compromise** — enterprises running agents on regulated workflows can't tell which actions were driven by trustworthy vs. poisoned context; a single compromise triggers wrong actions across unbounded sessions; company can't scope, disclose, or prove remediation
2. **Unprovable agent authorization** — regulated enterprises can't let agents operate autonomously because no one can prove actions stayed within the scope originally authorized; behavioral monitoring tells you when it failed, can't prove it succeeded; what's missing: verifiable proof at every tool call
3. **Ungovernable agent behavior** — agent behavior isn't a file; it's prompts + tools + permissions + retrieval sources + model config across multiple systems; security can't answer the 4 required questions (what changed, who approved, what new actions possible, what new data reachable); result: agents blocked from high-stakes workflows
4. TBD — Avishay
