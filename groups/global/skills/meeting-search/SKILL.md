...
`summary` | `transcript` | `insight`. Results are ordered by relevance rank, with summaries
and insights boosted above transcripts — so source order is a tendency, not a strict
guarantee. Matched terms in `snippet` are wrapped in `*...*`.
...
<url|Open>
```
Use the hit's `url` field directly for the link — don't hand-rebuild it.
...
- **No hits** → say so plainly and offer to broaden: drop a filter (participant/type/date) or
